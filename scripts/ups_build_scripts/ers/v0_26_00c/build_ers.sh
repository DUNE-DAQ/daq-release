...
function setup_and_verify()
{
    ...
    echo "${package} is installed at ${ERS_FQ_DIR}"
}
...
pkgtag="dune/ers-00-26-00-cmake"
...
case ${extraqual} in
debug) extra_command="-DCMAKE_BUILD_TYPE=Debug";;
prof)  extra_command="-DCMAKE_BUILD_TYPE="RelWithDebug;;
* )
...
git clone  https://github.com/philiprodrigues/ers || ssi_die "Unable to clone rogue git repo into ${PWD}"
cd ${pkgdir}/ers
git checkout ${pkgtag}
...
CXX=${cxx} \
    ...
    -DBOOST_ROOT=$ENV{BOOST_FQ_DIR} \
    -DCMAKE_POSITION_INDEPENDENT_CODE=TRUE \
    -DCMAKE_CXX_FLAGS=-I\ ${BOOST_INC}\ ${cxxflg} \
    ${pkgdir}/ers || ssi_die "Failed to run CMake"
...
#fi
rm -rf ${builddir}
mv ${pkgdir}/ers ${product_dir}/${package}/${pkgver}/source
