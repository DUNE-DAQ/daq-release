...
pushd drivers_rcc && git checkout a3a453e && popd
...
git clone https://:@gitlab.cern.ch:8443/atlas-tdaq-felix/ftools.git || ssi_die "Unable to clone external-catch git repo into ${PWD}"
pushd ftools && git checkout 0dc8aca && popd
...
cp x86_64-centos7-gcc8-opt/ftools/feconf* ${pkgdir}/bin
cp x86_64-centos7-gcc8-opt/ftools/femu* ${pkgdir}/bin
