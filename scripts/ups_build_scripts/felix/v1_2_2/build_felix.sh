...
flx_drivers_rcc_sha="c5c21e00"
flx_flxcard_sha="7614b8cc"
flx_regmap_sha="f0fd698e"
flx_packetformat_sha="a6933e36"
flx_client_template_sha="390ec871"
flx_ftools_sha="c326e788"
...
git clone https://gitlab.cern.ch/atlas-tdaq-felix/software.git || ssi_die "Unable to clone software git repo into ${PWD}"
cd software
git -c advice.detachedHead=false checkout ${flx_sofwtware_sha}

git clone https://gitlab.cern.ch/atlas-tdaq-felix/cmake_tdaq.git || ssi_die "Unable to clone cmake_tdaq git repo into ${PWD}"
pushd cmake_tdaq && git -c advice.detachedHead=false checkout ${flx_cmake_tdaq_sha} && popd
sed -i '2 i set(NOLCG TRUE)' cmake_tdaq/cmake/modules/FELIX.cmake
export PATH=${pkgdir}/software/cmake_tdaq/bin:$PATH

git clone https://gitlab.cern.ch/atlas-tdaq-felix/drivers_rcc.git || ssi_die "Unable to clone drivers_rcc git repo into ${PWD}"
pushd drivers_rcc && git -c advice.detachedHead=false checkout ${flx_drivers_rcc_sha} && popd
git clone https://gitlab.cern.ch/atlas-tdaq-felix/flxcard.git || ssi_die "Unable to clone flxcard git repo into ${PWD}"
pushd flxcard && git -c advice.detachedHead=false checkout ${flx_flxcard_sha} && popd
...
git clone https://gitlab.cern.ch/atlas-tdaq-felix/client-template.git || ssi_die "Unable to clone client-template git repo into ${PWD}"
pushd client-template && git -c advice.detachedHead=false checkout ${flx_client_template_sha} && popd
git clone https://gitlab.cern.ch/atlas-tdaq-felix/regmap.git || ssi_die "Unable to clone regmap git repo into ${PWD}"
pushd regmap && git -c advice.detachedHead=false checkout ${flx_regmap_sha} && popd
git clone https://gitlab.cern.ch/atlas-tdaq-felix/packetformat.git || ssi_die "Unable to clone packetformat git repo into ${PWD}"
pushd packetformat && git -c advice.detachedHead=false checkout ${flx_packetformat_sha} && popd
git clone https://gitlab.cern.ch/atlas-tdaq-felix/ftools.git || ssi_die "Unable to clone ftools git repo into ${PWD}"
pushd ftools && git -c advice.detachedHead=false checkout ${flx_ftools_sha} && popd
git clone https://gitlab.cern.ch/atlas-tdaq-felix/flxcard_py.git || ssi_die "Unable to clone flxcard_py git repo into ${PWD}"
pushd flxcard_py && git -c advice.detachedHead=false checkout ${flx_flxcard_py_sha} && popd
git clone https://gitlab.cern.ch/atlas-tdaq-felix/external-catch.git external/catch || ssi_die "Unable to clone external-catch git repo into ${PWD}"
pushd external/catch && git -c advice.detachedHead=false checkout ${flx_catch_sha} && popd
...
cp x86_64-centos7-gcc8-opt/flxcard_py/lib* ${pkgdir}/lib
