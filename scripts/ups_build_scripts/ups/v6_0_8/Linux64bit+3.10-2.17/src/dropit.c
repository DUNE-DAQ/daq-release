...
int
exp_stat(name, psb) 
char *name; struct stat *psb;
{
   static char replbuf[PATH_MAX];
   struct passwd *pw;
   char *psl;
   /* try to expand ~/ or ~user/ */
   if (name && name[0] == '~') {
       psl = strchr(name, '/');
       if (psl == name+1 || (psl == 0 && name[1] ==0)) {
           strcpy(replbuf, getenv("HOME"));
           if (psl) {
               strcat(replbuf, psl);
           }
           name = replbuf;
