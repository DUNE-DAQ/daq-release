...
class Rclone(Package):
    ...
    url = "https://github.com/rclone/rclone/archive/refs/tags/v1.64.0.tar.gz"
    ...
    def configure_args(self):
        args = ["--buildmode=c-shared",  "-o librclone.so"]
        return args

    def build(self, spec, prefix):
        #go(f"build --buildmode=c-shared -o librclone.so {self.stage.source_path}")
        go("build", "--buildmode=c-shared", '-o', 'librclone.so', join_path(self.stage.source_path, "librclone"))
        go("build")
