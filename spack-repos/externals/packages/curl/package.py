...
class Curl(NMakePackage, AutotoolsPackage):
    ...
    with when("tls=openssl"):
        pass   # JCF, Jan-14-2025: stripping out dependencies on Spack-installed openssl in our stack
        #depends_on("openssl")
    ...
    def with_or_without_openssl(self, activated):
        if self.spec.satisfies("@7.77:"):
            if activated:
                ...
                return "--with-openssl=/usr"  # JCF, Jan-14-2025: use system SSL
            else:
                return "--without-openssl"
        else:
            if activated:
                #return "--with-ssl=" + self.spec["openssl"].prefix
                return "--with-ssl=/usr"  # JCF, Jan-14-2025: use system SSL
            else:
                return "--without-ssl"
